...
- 保存、关闭并全新重开可编辑结果；
- 另一轮维护中的 Bridge 验收里，四阶段 Runtime 计划耗时 **4.312 秒**。
...
## 小修改直接出现在你正在看的 ADS 窗口里

![ADS 与 AEDT 受监督实时编辑的观测耗时](docs/assets/readme/supervised-live-edit-latency.png)

ADS 2026 Update 2.1 验收复用了同一个已打开图形进程。一个类型化 Patch
创建电阻和带标签连线并立即回读；保存或放弃仍由用户明确决定。对象事务观测
耗时为 **253 ms**；完全相同的回放用时 **3 ms**，重复对象为 0；仅回滚该
Patch 用时 **21 ms**，且只删除这次创建的对象。暖态参数修改端到端完成于
**93–187 ms**。

这些是 2026-08-31 在可丢弃工程中对 `0.1.0a48` 的受控观察，不是普遍延迟
保证。Codex 与 Pi Agent 均独立通过相同的创建、回放和回滚合同。详见
[脱敏实时编辑证据](docs/VALIDATION_2026-08-31_LIVE_EDIT.md)。
...
| “就在我正看着的设计里加这个元件并连线。” | 在精确匹配的活动 ADS 设计内执行幂等类型化 Patch，立即回读创建对象，并可在不保存的情况下只回滚该 Patch。 |
...
CSV 回读、原生 DDS 方程与曲线创建、已生成 Momentum 输入的执行，以及
已打开设计中的可逆实时对象 Patch。

另一轮维护中的 Bridge 验收里，“空白工作区 → 原理图 → 仿真 → 原生 DDS”
路径已作为一个四阶段 Runtime 计划通过：总耗时 **4.312 秒**，得到 31 个
有限数值点、确定命名的
