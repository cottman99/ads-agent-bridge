...
def validate_simulation_plan(value: Any) -> dict[str, Any]:
    ...
    if unknown:
        raise ValueError(
            "simulation plan contains unsupported fields: " + ", ".join(unknown)
        )
    required = (
        "schema_version",
        "operation_id",
    ...
    if plan["schema_version"] != _SCHEMA:
        raise ValueError(
            f"unsupported simulation plan schema: {plan['schema_version']}"
        )
    ...
    if dataset_name is not None and not re.fullmatch(
        r"[A-Za-z_][A-Za-z0-9_.-]{0,127}\.ds", str(dataset_name)
    ):
        raise ValueError("dataset_name must be a simple .ds filename")
    ...
    if dataset_name is not None:
        plan["dataset_name"] = str(dataset_name)
    return plan
